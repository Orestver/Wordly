...
btn_answer = tk.Button(frame_modes, text="20 слів", command=lambda:start_test("20"))
btn_answer.pack(side=tk.LEFT, padx=5)
style_button(btn_answer)

btn_infinity = tk.Button(frame_modes, text="Нескінченно", command=lambda:start_test("infinite"))
btn_infinity.pack(side=tk.LEFT, padx=5)
style_button(btn_infinity)

btn_all_words = tk.Button(frame_modes, text="Всі слова", command=lambda:start_test("all"))
...
repeat_wrong_words_btn.pack(pady=10)
...
cur_word_label = tk.Label(tab_wordstest, textvariable=var_word, font=("Arial",14), wraplength=500, justify=tk.LEFT, bg="#f5f5f5", state='normal')
cur_word_label.pack(pady=10)

entry_answer = tk.Entry(tab_wordstest, width=30, font=("Arial",12))
...
frame_buttons = tk.Frame(tab_wordstest, bg="#f5f5f5")
frame_buttons.pack(pady=5)

btn_answer = tk.Button(frame_buttons, text="Відповісти", command=check_answer)
btn_answer.pack(side=tk.LEFT, padx=5)
style_button(btn_answer)

btn_hint = tk.Button(frame_buttons, text="Підказка", command=give_hint_gui)
btn_hint.pack(side=tk.LEFT, padx=5)
style_button(btn_hint)

label_feedback = tk.Label(tab_wordstest, textvariable=var_feedback, font=("Arial",12), bg="#f5f5f5")
label_feedback.pack(pady=10)

progress_bar = ttk.Progressbar(tab_wordstest, orient="horizontal", length=500, mode="determinate",)
